...
func TestGetReceipts(t *testing.T) {
	...
	api := NewEthAPI(NewBaseApi(ff, stateCache, m.BlockReader, agg, false, rpccfg.DefaultEvmCallTimeout, m.Engine, m.Dirs, nil, nil), m.DB, nil, nil, nil, 1e18, 5000000, 100_000, false, 100_000, 128, log.New())
	...
	receipt, err := api.getReceipts(m.Ctx, rTx, block, []libcommon.Address{})
	require.NoError(t, err)
