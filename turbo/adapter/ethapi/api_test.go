...
func TestNewRPCTransactionDepositTx(t *testing.T) {
	...
	got := newRPCTransaction(tx, libcommon.Hash{}, uint64(12), uint64(1), big.NewInt(0))
	...
	require.Equal(t, got.Nonce, (hexutil.Uint64)(0), "newRPCTransaction().Mint = %v, want %v", got.Nonce, 0)
}
...
func TestUnmarshalRpcDepositTx(t *testing.T) {
	...
	for _, test := range tests {
		t.Run(test.name, func(t *testing.T) {
			...
			rpcTx := newRPCTransaction(tx, libcommon.Hash{}, uint64(12), uint64(1), big.NewInt(0))
			test.modifier(rpcTx)
