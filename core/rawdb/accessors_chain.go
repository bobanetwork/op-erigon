...
func ReadBody(db kv.Getter, hash common.Hash, number uint64) (*types.Body, uint64, uint32) {
	...
	body.Rejected = bodyForStorage.Rejected
...
func ReadBlock(tx kv.Getter, hash common.Hash, number uint64) *types.Block {
	...
	return types.NewBlockFromStorage(hash, header, body.Transactions, body.Uncles, body.Withdrawals).WithRejected(body.Rejected)
}
