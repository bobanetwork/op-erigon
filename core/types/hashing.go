...
func DeriveSha(list DerivableList) common.Hash {
	...
	traverseInLexOrder(list, func(i int, next int) {
		...
		if curr.Len() > 0 {
			...
			groups, branches, hashes, _ = trie.GenStructStep(false, nil, nil, retain, curr.Bytes(), succ.Bytes(), hb, nil /* hashCollector */, &leafData, groups, branches, hashes, false, false)
		}
	})
