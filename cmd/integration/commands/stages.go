...
func stageTrie(db kv.RwDB, ctx context.Context) error {
	...
	if unwind > 0 {
		...
		if _, err := stagedsync.SpawnIntermediateHashesStage(s, sync /* Unwinder */, tx, cfg, ctx, false /* quiet */, chain2.Config{}); err != nil {
			return err
		}
	}
...
func newSync(ctx context.Context, db kv.RwDB, miningConfig *params.MiningConfig) (consensus.Engine, *vm.Config, *stagedsync.Sync, *stagedsync.Sync, stagedsync.MiningState) {
	...
	miningSync := stagedsync.New(
		stagedsync.MiningStages(ctx,
			...
			stagedsync.StageMiningExecCfg(db, miner, events, *chainConfig, engine, &vm.Config{}, dirs.Tmp, nil, 0, nil, nil, allSn, cfg.TransactionsV3, nil, nil),
			...
			stagedsync.StageMiningFinishCfg(db, *chainConfig, engine, miner, miningCancel, nil),
		),
